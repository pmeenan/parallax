...
export function streamingResourceCacheKey(descriptor: StreamingDependencyIndexEntry): string {
  const decode =
    descriptor.format === "ktx2"
      ? `v${descriptor.decode.version ?? 0}:${descriptor.decode.format}:${descriptor.decode.colorSpace}:${descriptor.decode.width}x${descriptor.decode.height}${descriptor.decode.version === 2 ? `:mips=${descriptor.decode.mipLevelCount}` : ""}`
