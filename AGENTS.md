...
- **Budgets are enforced, not eternal.** [docs/budgets.md](docs/budgets.md) defines the
  performance budgets. Focused and specialized checks run with the work they cover;
  the full physical `smoke@1` gate runs once against the exact milestone-exit candidate
  (D-181). A milestone whose exit gate busts an applicable budget is not done. Budgets
  themselves are recalibrated as measurements come in — through a decision-log entry,
  never by weakening a check to make a change pass.
...
12. **Use local creative tools.** Build 3D assets with local Blender through seeded
    headless scripts ([assets/production-workflow.md](assets/production-workflow.md));
    the Blender MCP server, when connected, is for interactive work. Generate images with the agent's image-generation capabilities
...
(install/launch/run lifecycle + caches), M3 (gameplay core + NPC AI), M3.5
(gameplay systems), and M4 (District 2 + hard transitions) are complete**. **M4.5
(first finished playable area; scene-driven rendering research, D-182) is in progress.**
