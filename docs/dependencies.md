...
| `@huggingface/transformers` | 4.2.0 | 4.2.0 | **Retained; current stable.** The D-073 ONNX path remains negative evidence rather than the production direction. | 2026-08-17 or a release fixing the recorded ONNX failures. |
...
| ONNX Runtime Web override | 1.27.0 | 1.27.0 stable (1.28 development builds only) | **Retained; current stable.** The resolved optional Node binding remains 1.24.3 and is not shipped to the browser. | 2026-08-17 or a stable release fixing RE-030/031/032. |
| Gemma model inputs | ONNX QAT `5cd5514e…`, ONNX WASM `9f4bef82…`, GGUF `66a399f6…`; exact artifact hashes in source | The Hugging Face API reported the same three repository heads | **Retained.** Exact revisions, tokenizer/chat templates, GGUF shards, sizes, and SHA-256 manifests remain unchanged. | 2026-08-17 or an upstream revision/model candidate with a documented quality, kernel, or cache-correctness reason. |
...
| Type packages | `@types/node` 24.13.3; `@types/dom-chromium-ai` 0.0.17 | Same within the selected Node 24/API lines (`@types/node` 26 is for a different runtime major) | **Retained; current/applicable.** | 2026-08-17 or a typing fix for an API Parallax uses. |

### Advisory disposition

`pnpm audit --json` reported GHSA-xcpc-8h2w-3j85 (CVE-2026-39244), high severity,
for `adm-zip` 0.5.18 under `@huggingface/transformers` → optional
`onnxruntime-node` 1.24.3. The advisory affects extraction of attacker-controlled ZIPs
and is patched in `adm-zip` 0.6.0, outside `onnxruntime-node`'s declared `^0.5.16`
range. Local source inspection found the only ONNX consumer in `script/install-utils.js`,
reached from the package postinstall; this repository explicitly blocks
`onnxruntime-node` builds in `pnpm-workspace.yaml`, and neither that Node binding nor
`adm-zip` is present in the served browser manifest. The finding is therefore
**not applicable to shipped/runtime code but retained as an installation-tree risk**;
do not force an out-of-range transitive override without upstream compatibility
evidence. Recheck immediately when ONNX Runtime publishes a compatible patched chain,
when the blocked build policy changes, or when the ONNX experiment is removed.
