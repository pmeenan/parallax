...
New releases wait out a supply-chain window scaled to the size of the change (human direction,
2026-09-24):
- **Patch** (`x.y.Z`, usually a fast-follow fix): any age.
- **Minor** (`x.Y.z`): the new minor line's first release is at least 1 day old.
- **Major** (`X.y.z`, or a `0.Y.z` minor, which is breaking under semver): the new major line's
  first release is at least 7 days old.
- **New dependency:** the exact version is at least 7 days old.

The window belongs to the release that opens a line, so patches on an opened line are free. The
verdict also does not depend on which intermediate pins were committed. For example, moving from
1.12.0 to 1.31.1 needs 1.31.0 to be a day old, and 1.31.1 may be any age.

pnpm 11 has one window (`minimumReleaseAge`, pinned at 1440 minutes in `pnpm-workspace.yaml`)
and accepts only exact `name@version` exclusions. So after changing any pin, run
`node harness/scripts/release-age-gate.mjs --write`. It compares every exact pin with `git HEAD`,
checks the registry publish times of each changed package against its tier, and fails on a
violation. It adds the exact exclusion pnpm needs for an allowed pin younger than a day, and
removes exclusions for versions no longer pinned.

The tiers set only the supply-chain wait, not the review. Upstreams do not all follow semver:
Babylon Lite 1.31.1 is a patch that lists two breaking changes and four features across 255
changed files. The tier gates below apply to every adopted version, patches included.
...
4. Upgrade one runtime-critical component family at a time, and pass the release-age gate
   (`node harness/scripts/release-age-gate.mjs --write`). Preserve an old-pin build or
...
| --- | --- | --- |
| Babylon Lite device-loss seam (D-104) | Pinned 1.31.1 still needs the bounded `lite-device-loss.ts` seam to observe and force loss through Lite's private `_device`, while Parallax restarts the complete render/stream/decode cohort. Lite 1.14.0 added public device-loss recovery controls (`enableDeviceLostSceneRecovery`), but its in-place resource rebuild is not equivalent to that product recovery contract. Its force-loss test hook was still not public at 1.31.0 and 1.31.1. | Review with every renderer-family change. Retire or narrow the seam only after public API/source inspection proves equivalent observe/force-loss coverage, render-recovery fixtures pass, and the required same-scenario physical renderer gate passes. The existence of a public in-place recovery API alone is not a retirement trigger. |

## Review ledger

| Review date | Scope and outcome | Next checkpoint |
| --- | --- | --- |
| 2026-09-24 | **Renderer-family patch review:** adopted Babylon Lite 1.31.1 (from 1.31.0) under the tiered release-age policy, with the exact pnpm exclusion `@babylonjs/lite@1.31.1`. The patch lists two breaking changes (material `Float32Array` uniform access, inspector parity accessors) and four features across 255 changed files. The review found no API or private-internal break for Parallax: `tsc -b` passed and all ten private fields read by the engine are still present. Two composed-WGSL pins moved, the PBR colour and PBR depth fragments, because the template now declares `metallic` and `occlusion` with `var` rather than `let`. Reverting those two declarations in the captured WGSL reproduces the 1.31.0 hashes exactly, and the Standard pins are unchanged. Other changes relevant to Parallax: the per-task GPU timer now resolves only for its own encoder; removing a mesh now drops its dispose closure from `scene._disposables`, which previously grew until scene disposal under streaming eviction; and many error codes were renumbered, none of which the engine matches. Evidence on build `4ea1b8c6…`: `pnpm check`; installer-repair replay `…19-39-32-026Z`; installed scale-streaming `…19-44-39-848Z` (cell-load p95 1.79 ms, against 1.96 ms on 1.31.0); animation import `…19-43-59-478Z`; and physical `render-recovery@2` `…19-38-56-026Z`, with all facets and the contract passed. Paving captures match 1.31.0 pixel for pixel in all eight views, excluding the live diagnostic overlay. The paving load is 218.3 ms against 218.6 ms, and CPU submit is 0.24–0.30 ms p50 against 0.27–0.35 ms. After review fixes (texture cache key, restored raw-KTX2 checks), build `d98b6c89…` passed the installer-repair replay `…20-22-05-358Z`, physical `render-recovery@2` `…20-32-36-884Z` and installed scale-streaming `…20-32-37-264Z` (cell-load p95 2.14 ms). Deferred: `@babylonjs/ktx2decoder` 9.28.0, a minor release eligible from 2026-09-25T07:43Z. Physical `smoke@1` stays at M4.5 exit (D-181). | Next renderer-feature package, or 2026-10-22. |
| 2026-09-24 | **Targeted renderer-family review:** adopted Babylon Lite 1.31.0 (from 1.12.0) and `@babylonjs/ktx2decoder` 9.27.1 (from 9.17.0), the newest releases outside pnpm's 24-hour minimum release age. No age exception was added, and the stale `@babylonjs/lite@1.12.0` exclusion was removed. The full 1.13.0–1.31.0 changelog was reviewed. All 58 Lite exports the engine uses keep compatible signatures, and every private internal read is still present. Adaptations: the PSO-warmup observer checks the 1.20/1.22 opt-in material fields; seven composed-WGSL pins were recaptured from pinned Chrome, with unchanged pipeline structure; renderer identities and version-pinned tests were re-verified; decoder literals were updated; raw RGBA8 now uses the decoder. Evidence on build `51b16294…`: `pnpm check`, installer-repair replay `…18-47-00-230Z`, installed scale-streaming `…18-48-27-913Z` (traversal p95 1.96 ms) and animation import all passed. Courtyard captures are pixel-identical to 1.12.0 (0.000 mean), and the paving load is 218 ms against 231 ms. CPU submit rose from 0.16–0.20 to 0.27–0.33 ms p50. A live probe showed device-loss recovery rendering identically before and after. The render-recovery and flythrough harnesses failed on the old pin as well, for stale-harness causes filed as separate tasks. Physical `smoke@1` stays at M4.5 exit (D-181). | Next renderer-feature package, or 2026-10-22. |
| 2026-09-11 | **Animation source QA:** added exact development-only `gltf-validator` 2.0.0-dev.3.10, the current npm release observed, from the official Khronos package. Its Apache-2.0 license, exported validation API and documented schema/binary/animation checks were inspected. No runtime renderer/decoder pins changed. The source gate rejects extensions outside its supported core profile and retains validator messages. Static corruption fixtures, pinned Babylon worker loading and repository checks accompany adoption; this does not reset the full currency checkpoint. | A validator advisory/release, source-profile extension, or next full checkpoint. |
| 2026-09-11 | **Immediate audit triage during animation work:** `pnpm audit` reports two moderate records for existing Vitest/mocker 4.1.10 ([upstream advisory](https://github.com/vitest-dev/vitest/security/advisories/GHSA-82fw-gwwq-j7x9), fixed in 4.1.11) and one high record for existing sharp 0.35.3 ([upstream advisory](https://github.com/lovell/sharp/security/advisories/GHSA-rgj7-g3m4-5g8c), fixed in 0.35.4). Inspected affected paths: this task uses Node Vitest without the standalone mocker plugins, and sharp receives raw worker RGBA for PNG encoding, not HEIF input. No advisory targets the new Khronos package. Defer the two existing tool-family upgrades to isolated dependency work with their required fixture/check gates; audit is not clean. | Upgrade/review before exposing standalone mocker services or processing untrusted HEIF/AVIF, and no later than the 2026-09-19 checkpoint. |
| 2026-09-05 | **D-189 shared LFS infrastructure:** selected Rudolfs 0.3.8, the latest numbered upstream/Docker release observed (image published 2025-06-08), by immutable digest in `deploy/lfs/compose.yaml`; newer floating `master` is not selected. Verified executable version and inspected tagged local-storage/API implementation. Qualified nginx access boundaries, hash rejection, GET/HEAD, batch URLs and a streamed 2 GiB + 17 byte round trip on plex. This is external supporting infrastructure, not a game/runtime dependency. Production installation, trusted TLS, LAN Git LFS upload/cold-cache checkout, and Cloudflare batch/download/access checks subsequently passed; no claim of a full container vulnerability audit. | Next dependency checkpoint, a numbered Rudolfs release/security advisory, backend change, or access-policy change. |
| 2026-08-22 | **Accepted full repository checkpoint for M3.5 exit and M4 entry (D-173):** adopted CfT Stable 152.0.7977.54 after the exact M3.5 artifact passed the complete six-launch physical contract on both retained 151.0.7922.108 and current 152. The game/runtime, model, renderer/decoder, Rust/WASM, fixture, and build-tool candidates below remain exact-current or deferred to isolated component-family gates. `pnpm audit --json` reports zero advisories across 211 dependencies. | This checkpoint satisfies M4 entry. Next full checkpoint: M4 exit, M4.5 entry, or 2026-09-19, whichever comes first; relevant security notices and row-specific triggers remain immediate. |
...
## Full checkpoint — 2026-08-22 (M3.5 exit + M4 entry)

This review re-enumerated the direct package graph, Node/pnpm, CfT, Rust/WASM tools and
resolved crates, selected model/runtime and exact model artifacts, decoder binaries,
generation tools, pinned fixture, and registered machine state. Official release,
registry, advisory, source, model, fixture, and Chrome feeds were checked on 2026-08-22.
Chrome advanced during the exit qualification, so the same exact artifact was measured
on old and new pins and 152.0.7977.54 is adopted. No other candidate is coupled to that
browser transition. The next full checkpoint is M4 exit, M4.5 entry, or 2026-09-19,
...
| --- | --- | --- | --- |
| CfT Stable 151.0.7922.108, revision 1654411 | 152.0.7977.54, revision 1669021 | **Adopted and current after D-173.** The win64 archive/executable SHA-256 are `91850065e6b80bba0c752e17a150fe1b9e39bba51ed705640c1273f565950dda` / `b0123437c55a3893e8988328f576ffcbe68cee7743d3653ffe865c73633b1ef4`; CDP reports `Chrome/152.0.7977.54` at `@24072c1aa400ec4a89dc738b6b6acd12a8589b6f`. The exact M3.5 artifact passed schema-v72 / metric-set-v35 physical smoke on both versions. The checked-in `.108` descriptor and local binary remain a reconstruction anchor. | The next Stable advance, an advisory, M4 exit, or a release relevant to an open Chrome finding. |
| `@babylonjs/lite` 1.31.1 | Same release, the latest (registry checked 2026-09-24) | **Adopted 2026-09-24** as a patch review on top of the 1.31.0 adoption (from 1.12.0); see the ledger. The release-age gate added the exact pnpm exclusion `@babylonjs/lite@1.31.1`, because the release was 14 hours old. Scene pixels are identical to 1.31.0; CPU submit sits about 0.1 ms per frame above 1.12.0. | The next renderer-feature package, or 2026-10-22. |
| `@babylonjs/ktx2decoder` 9.27.1 and package-owned WASM | 9.28.0 (registry checked 2026-09-24) | **Adopted 2026-09-24** (from 9.17.0). Its WASM binaries are byte-identical to 9.17.0's. Its uncompressed RGBA8 transcoder replaced the engine-owned `ktx2-rgba8.ts` reader. 9.28.0 is a minor release, eligible 1 day after its 2026-09-24 publish. **Coupled asset (D-201):** pre-encoded BC7 maps are made at pack time by this package's `uastc_bc7.wasm`. The library records which version (`encoders.bc7Transcoder`: version and wasm SHA-256). Each decoder upgrade repacks and readmits those maps so they pick up transcoder fixes and improvements, and regenerates `production-compressed-fixtures.generated.ts`. Older output is still valid BC7, so the build only warns while a pack lags the pin. | With the next Lite review: exact binary hashes, wrapper/API inspection, decoder fixtures, and a repack of pre-encoded BC7 assets. |
| `@types/webxr` 0.5.24 (root dev) | Same release | **Added 2026-09-24.** Lite 1.31's typings reference WebXR globals. TypeScript 7 does not auto-include `@types`, so `engine`, `harness`, `assets` and the tests project list it. | A release, or the next full checkpoint. |
| `draco3dgltf` 1.5.7; `meshoptimizer` 1.2.0 | Same releases | **Current.** Package-owned copied decoder identities remain selected. | A release/advisory, representative content, or next full checkpoint. |
| `@noble/hashes` 2.2.0 | 2.3.0 | **Deferred.** Integrity-path performance and type changes require D-133's isolated hash/OPFS comparison. | An advisory, integrity work, or 2026-09-19. |
| `@wllama/wllama` 3.5.1 and pinned llama.cpp commit | 3.6.0 | **Deferred.** The new runtime remains a model/AI-family transition with worker, WebGPU, memory, and generated-artifact gates; M3.5 does not exercise inference. | Approved model-runtime work, an advisory, or 2026-09-19. |
| Gemma 4 E2B QAT GGUF revision `66a399f68ddd113b06dff02fca9523e55465d11d` and five exact installed shards | Upstream repository head `675cff42a74c774d6cb76f76d8eacb49b48c9b93` | **Deferred.** An upstream head change does not replace Parallax's exact model, tokenizer/chat-template, split hashes, or accepted provisioning contract without a model-family quality/performance gate. | Approved model/runtime work, an advisory, or 2026-09-19. |
| Node 24.18.1 | 24.19.0 LTS | **Deferred.** The non-security runtime update can alter every emitted artifact and harness process; audit is clean and no M3.5 fix requires it. | A Node security notice, an isolated build comparison, or 2026-09-19. |
| Rust `nightly-2026-07-16` | nightly-2026-08-22 | **Deferred.** Compiler, allocator, atomics, SIMD, and generated layout remain an isolated Rust/WASM transition. | Rust/WASM work, a security trigger, or 2026-09-19. |
| wasm-bindgen crate + CLI 0.2.127 | 0.2.127 | **Current.** The D-164 layout fix remains selected and the recurring exact generated-Wasm checks pass. | A release/advisory or layout failure. |
| Binaryen/npm 131.0.0 | 132.0.0 | **Deferred.** The skipped release changes relaxed-atomics handling and JavaScript APIs and can change optimized Wasm bytes. | A WASM pipeline change, advisory, or 2026-09-19: skipped-range review, exact layout and byte comparison, repeatability, and physical smoke if bytes move. |
...
| Selected input | Candidate checked | Outcome and evidence | Next recheck / required gate |
| --- | --- | --- | --- |
| pnpm 11.12.0 | 11.22.0 | **Deferred.** Lock/install policy and artifact identity require an isolated package-manager comparison. | Before the next dependency update or 2026-09-19. |
| Vite 8.1.4 | 8.2.2 | **Deferred.** Build lifecycle, resolution, and output-byte changes remain an isolated build-family transition. | M4 build work or 2026-09-19. |
| Playwright Core 1.61.1 | 1.62.1 | **Deferred.** Launch, trace, screenshot, and mandatory-metric semantics move as one automation family. Chrome 152 passed with the selected version. | A collector/browser trigger or 2026-09-19. |
| `sharp` 0.35.3 | 0.35.3 | **Current.** | A release/advisory or screenshot-format change. |
| Transitive nanoid 3.3.18 through PostCSS/Vite | Selected compatible resolution; no advisory candidate | **Current.** `pnpm audit --json` reports zero advisories. | A nanoid/PostCSS/Vite advisory or owning dependency change. |
| `@biomejs/biome` 2.5.3 | 2.5.10 | **Deferred.** Parser, formatter, lint, and diagnostics changes remain a supporting-tool transition. | Before a formatting migration or 2026-09-19. |
| `@types/node` 24.13.3 | Current 26.x line | **Not applicable to the selected Node 24 runtime.** | Node-major transition. |
| Rollup 4.62.2 | 4.62.5 | **Deferred.** Bundler fixes require an isolated emitted-artifact comparison. | Relevant build work or 2026-09-19. |
| esbuild 0.28.1 | 0.28.2 | **Deferred.** Code generation/tree-shaking changes require an isolated artifact comparison. | Relevant build work or 2026-09-19. |
| Vitest 4.1.10 | 4.1.11 | **Deferred.** Keep the test-runner move out of the browser and gameplay-exit evidence unit. | A test-runner issue or 2026-09-19. |
| TypeScript 7.0.2; `@rollup/plugin-node-resolve` 16.0.3 | Same releases | **Current.** | A release/advisory or next full checkpoint. |

`pnpm audit --json` reports zero critical, high, moderate, low, or informational
advisories across 211 dependencies. Final qualification used exact Node 24.18.1 and
pnpm 11.12.0; `pnpm check` passed repeatable production output, lint, 202 test files,
and 2,568 tests (one skipped). Exact build/install identities and both passing physical
Chrome reports are recorded in D-173. The Chrome-152 report's streaming relative-range
diagnostic remained informational under D-154; every per-launch absolute streaming
budget and all 36 mandatory checks passed. No threshold, baseline, audit suppression,
or floating dependency range changed.
...
[Chrome 152 platform notes](https://developer.chrome.com/blog/new-in-webgpu-151-152),
[Node releases](https://nodejs.org/dist/index.json), npm registry and advisory metadata,
the [Binaryen changelog](https://github.com/WebAssembly/binaryen/blob/main/CHANGELOG.md),
upstream GitHub release/commit APIs for wasm-bindgen, wllama, KTX-Software, and the
Khronos fixture, the selected Hugging Face model repository API, the Rust nightly
channel manifest, and local registered-machine inspection.
