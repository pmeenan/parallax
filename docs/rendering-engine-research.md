...
| KTX2 BCn passthrough | missing (`@babylonjs/ktx2decoder` 9.27.1) | The decoder classifies any colour model other than UASTC or RGBA as ETC1S. It cannot return the stored levels of a `BC1`/`BC7` KTX2, and its UASTC→BC1 target is a full BC1 encode, about 8× slower than UASTC→BC7. Parallax validates and slices raw BC1 levels itself (D-201). Candidate upstream: a passthrough for block-compressed `vkFormat`s. |
