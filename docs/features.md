...
| Animation content | Source GLB import, rig/skin/clip validation and pinned-worker checks implemented; actual rigged NPC/enemy locomotion/idle/combat and character admission remain M4.5 work. Expand across body types in M5 (D-141/D-182) | import foundation implemented; content pending | M4.5 first set, M5 expansion |
| Cinematics & scripted cameras | Sealing-choice epilogue uses normal gameplay/dialogue framing, as selected by the human on 2026-09-19. No dedicated epilogue camera sequence; other cinematic work needs a separate named scene requirement. | designed | M6 presentation |
...
| Spatial audio | Bounded native positional playback, camera listener, semantic-event hooks and lifecycle telemetry integrated; bank empty. QA-admitted SFX/ambience, installed audio loading and surface/underground audition remain open. Expand worklet/HRTF/acoustic coverage in M6 (D-182); see [foundation](spatial-audio.md). | foundation implemented; content pending | M4.5 first route, M6 expansion |
