...
| Quests & journal | D-170 supplies validated saved one-time landmark discovery and shared progression XP. D-171 adds the data-owned six-stage main arc and eight side quests, semantic-event-only typed objectives, validated intent/delivery commands, bounded encounter-preparation consequences, shared quest XP, and a fail-closed append-only save-schema-v10 journal with paginated localization-ready queries. | implemented (simulation; hybrid-UI consumer next) | M3.5 |
