...
| Game UI stack | D-160 selects a measured hybrid over the worker-owned WebGPU canvas: world anchors and heavy-screen visuals/interactions render in-canvas; HUD and dialog use DOM/CSS; in-canvas screens may expose a sparse DOM semantic/IME bridge without duplicating their visual tree. D-143 identified no page-visible cross-thread presentation synchronization/attribution primitive; RE-047 keeps that request open. | active (substrate decided) | M3 |
