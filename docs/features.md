...
| Combat, progression & crafting | Full mechanics stack inside the deterministic sim worker — replays double as gameplay regression tests; D-166 adds authored navigation-aware pack AI, serialized behavior/boss state, semantic danger transitions, D1 greybox spawns, and measured encounter readability through facing gates, encounter attack slots, stable flanks, and warned hazards; original slice-scale ruleset, no D&D-protected material (D-141) | active | M3.5 |
