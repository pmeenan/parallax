...
OUT=ROOT/"harness/results/d1-paving-proof-2026-09-20/cycle1"
...
noise=n.new("ShaderNodeTexNoise");noise.inputs["Scale"].default_value=6.5;noise.inputs["Detail"].default_value=2.2;noise.inputs["Roughness"].default_value=.65
l.new(uv.outputs["Object"],noise.inputs["Vector"])
ramp=n.new("ShaderNodeValToRGB");ramp.color_ramp.elements[0].position=.28;ramp.color_ramp.elements[0].color=(.42,.43,.40,1)
ramp.color_ramp.elements[1].position=.73;ramp.color_ramp.elements[1].color=(.96,.89,.73,1)
...
# Dense, gently irregular continuous earth, covering buried sides and edging out beyond4m.
N=180;vs=[];fs=[]
for j in range(N+1):
 y=-2.35+4.7*j/N
 for i in range(N+1):
  x=-2.35+4.7*i/N;vs.append((x,y,ground_h(x,y)))
for j in range(N):
 for i in range(N):
...
def free_radius(x,y):
 best=1
 for a,b,c,d in hulls:
  if a<=x<=b and c<=y<=d:return -1
  dx=max(a-x,0,x-b);dy=max(c-y,0,y-d);best=min(best,math.hypot(dx,dy))
 return best
# Geometry grit: shared primitive meshes, varied buried fragments only in open soil.
gritmeshes=[]
for q in range(4):
 bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=1,radius=1)
 o=bpy.context.object;o.data.materials.append(gritmat);gritmeshes.append(o.data);o.data.use_fake_user=True;bpy.data.objects.remove(o,do_unlink=True)
grit=0
for attempt in range(26000):
 if grit>=1800:break
 x,y=rng.uniform(-2.15,2.15),rng.uniform(-2.15,2.15);radius=rng.uniform(.002,.009)
 if free_radius(x,y)<radius*.65:continue
 o=bpy.data.objects.new("Embedded grit",gritmeshes[grit%4]);scene.collection.objects.link(o)
 o.location=(x,y,ground_h(x,y)-radius*.15);o.scale=(radius,radius*rng.uniform(.6,1),radius*rng.uniform(.45,.8));o.rotation_euler=(rng.random(),rng.random(),rng.random()*6.28);grit+=1
# Sparse rosettes: visibly broad leaves with a central ridge, shared rooted crown.
roots=[];leafobjects=[]
for attempt in range(10000):
 if len(roots)>=22:break
 ...
 z=ground_h(x,y)-.001;roots.append((x,y,z));scale=rng.uniform(.7,1.25)
 for leaf in range(rng.randint(5,8)):
  ...
  vs=[];fs=[]
  for s in range(9):
   t=s/8;ww=width*math.sin(math.pi*t)**.75
   for side in [-1,0,1]:
    along=length*t;sideways=side*ww
    # Lift leaves above low stone tops without moving the rooted crown.
    hh=z+.027*math.sin(t*math.pi/2)+rise*math.sin(math.pi*t*.8)-abs(side)*.004*math.sin(math.pi*t)
    vs.append((x+math.cos(ang)*along-math.sin(ang)*sideways,y+math.sin(ang)*along+math.cos(ang)*sideways,hh))
  for s in range(8):
   for t in range(2):
    a=s*3+t;fs.append((a,a+1,a+4,a+3))
  ...
  poly=curve.splines.new("POLY");poly.points.add(8)
  for s in range(9):
   p=Vector(vs[s*3+1]);p.z+=.0004;poly.points[s].co=(*p,1)
...
cam.data.type="ORTHO";cam.data.ortho_scale=4.7;cam.location=(0,0,6);cam.rotation_euler=(0,0,0)
