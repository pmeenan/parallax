...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    p[:,2]-=top*(.003+.028*noise(p,22,seed+76)**2)*np.clip(1-edge_distance/edge_width,0,1)**.8
...
def stone_material():
    ...
    l.new(mix.outputs[0],detail.inputs[1]);l.new(image.outputs['Color'],detail.inputs[2]);l.new(albedo.outputs['Color'],bs.inputs['Base Color'])
