...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    edge_width=.002+.028*noise(p,32,seed+75)**2
    p[:,2]-=top*(.001+.009*noise(p,28,seed+76)**2)*np.clip(1-edge_distance/edge_width,0,1)**.8
    # Unequal side batter reduces the extruded-cuboid reading.
    level=(raw[:,2]+hz)/(2*hz)
    for axis in (0,1):p[:,axis]*=1-(.015+.026*noise(p,17,seed+80+axis))*(1-level)
