...
assert MACRO.packed_file and max(MACRO.pixels[:128])>.1, 'Scalar image must survive initialization and packing'
...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    scalar=height_sample(p,norm)
    macro_scalar=height_sample(p,norm,HEIGHT_MACRO)
    # Pits occupy lower chalky matrix between smoother, mineral-rich plateaus.
    top=np.clip(norm[:,2],0,1)
    relief=(np.minimum(scalar,.64)-.57)*.005+(broad-.5)*.0012
    ...
    shoulder_loss=(.003+.028*noise(p,22,seed+76)**2)*np.clip(1-edge_distance/edge_width,0,1)**.8
...
def stone_material():
    ...
    mapping.inputs[2].default_value=(.5,.5,.5-.04/.45)
    ...
    albedo=n.new('ShaderNodeTexImage');albedo.image=bpy.data.images.load(str(ROOT/'inputs/limestone-albedo-v1.png'),check_existing=True)
    albedo.projection='BOX';albedo.projection_blend=.2
    l.new(mapping.outputs[0],albedo.inputs['Vector'])
    col=ramp(n,l,macro_tex.outputs['Color'],'Shared sculpt field mineral interpretation',[
        (.25,(.24,.185,.115)),(.50,(.44,.36,.245)),(.59,(.34,.305,.23)),(.65,(.19,.195,.168)),(.73,(.165,.173,.15)),(.86,(.33,.265,.18))])
    ...
    l.new(col,mix.inputs[1]);l.new(mineral,mix.inputs[2])
    image=n.new('ShaderNodeTexImage');image.image=bpy.data.images.load('D:/src/parallax/assets/reference/d1-paving-clean/limestone-single-surface-v1.png',check_existing=True)
    image.projection='BOX';image.projection_blend=.25
    mapping=n.new('ShaderNodeVectorMath');mapping.operation='SCALE';mapping.inputs['Scale'].default_value=2
    l.new(coord.outputs['Object'],mapping.inputs[0]);l.new(mapping.outputs['Vector'],image.inputs['Vector'])
    detail=n.new('ShaderNodeMixRGB');detail.blend_type='MULTIPLY';detail.inputs[0].default_value=.50
    l.new(mix.outputs[0],detail.inputs[1]);l.new(image.outputs['Color'],detail.inputs[2])
    tint=n.new('ShaderNodeMixRGB');tint.blend_type='MULTIPLY';tint.inputs[0].default_value=1;tint.inputs[2].default_value=(.66,.64,.61,1)
    l.new(albedo.outputs['Color'],tint.inputs[1]);l.new(tint.outputs[0],bs.inputs['Base Color'])
    rough=n.new('ShaderNodeMapRange');rough.inputs['To Min'].default_value=.9;rough.inputs['To Max'].default_value=.62
    l.new(height_tex.outputs['Color'],rough.inputs['Value']);l.new(rough.outputs[0],bs.inputs['Roughness'])
    ...
    bump.inputs['Distance'].default_value=.00016;bump.inputs['Strength'].default_value=.5
