...
OUT=ROOT/f'portable-cycle{CYCLE}';OUT.mkdir(exist_ok=True)
START=time.perf_counter()
source_scene=bpy.data.scenes[f'Complete limestone study cycle{CYCLE}']
...
high=next(o for o in scene.objects if o.name.startswith('Surface study master'))
...
receipt={'createdUtc':datetime.now(timezone.utc).isoformat(),'blender':bpy.app.version_string,
 'source':f'cycle{CYCLE}/source.blend','triangles':len(low.data.polygons),'vertices':len(low.data.vertices),
