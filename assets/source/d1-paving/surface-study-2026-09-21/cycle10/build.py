...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    relief=(np.minimum(scalar,.61)-.57)*.0045+(broad-.5)*.0012
...
def stone_material(seed,texture_offset):
    ...
    col=ramp(n,l,macro_tex.outputs['Color'],'Quiet gray worn mineral and fresh cream matrix',[
        (.25,(.22,.175,.115)),(.43,(.12,.115,.105)),(.58,(.13,.13,.115)),(.67,(.34,.28,.21)),(.76,(.56,.46,.32)),(.88,(.62,.51,.37))])
    ...
    mix=n.new('ShaderNodeMixRGB');mix.blend_type='MIX';mix.inputs[0].default_value=.58
    ...
    bump=n.new('ShaderNodeBump');bump.inputs['Distance'].default_value=.00013;bump.inputs['Strength'].default_value=.45
