...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    relief=(np.minimum(scalar,.58)-.57)*.0035+(broad-.5)*.0012
...
def stone_material():
    ...
    col=ramp(n,l,macro_tex.outputs['Color'],'Quiet gray worn mineral and fresh cream matrix',[
        (.25,(.22,.175,.115)),(.43,(.20,.19,.165)),(.58,(.21,.205,.19)),(.67,(.38,.32,.23)),(.76,(.56,.46,.32)),(.88,(.62,.51,.37))])
    ...
    mix=n.new('ShaderNodeMixRGB');mix.blend_type='MIX';mix.inputs[0].default_value=.28
    l.new(col,mix.inputs[1]);l.new(tint.outputs[0],mix.inputs[2]);l.new(mix.outputs[0],bs.inputs['Base Color'])
    ...
    bump=n.new('ShaderNodeBump');bump.inputs['Distance'].default_value=.00009;bump.inputs['Strength'].default_value=.4
