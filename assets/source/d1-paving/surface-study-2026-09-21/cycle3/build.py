...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    relief=(np.minimum(scalar,.76)-.57)*.008+(broad-.5)*.0012
    side_relief=(np.minimum(scalar,.76)-.57)*.009+(noise(p,46,seed+40)-.5)*.0015
...
def stone_material():
    ...
    col=ramp(n,l,height_tex.outputs['Color'],'Shared sculpt field mineral interpretation',[
        (.15,(.16,.135,.095)),(.36,(.34,.28,.19)),(.49,(.43,.37,.265)),(.61,(.26,.255,.215)),(.73,(.195,.20,.178)),(.86,(.44,.37,.265))])
