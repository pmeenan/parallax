...
def height_sample(p,normal):
    ...
    for axis,(a,b) in enumerate(((1,2),(0,2),(0,1))):
        ...
        result+=weights[:,axis]*((HEIGHT_PIXELS[y,x]*(1-fx)+HEIGHT_PIXELS[y,np.minimum(x+1,HEIGHT.size[0]-1)]*fx)*(1-fy)+(HEIGHT_PIXELS[np.minimum(y+1,HEIGHT.size[1]-1),x]*(1-fx)+HEIGHT_PIXELS[np.minimum(y+1,HEIGHT.size[1]-1),np.minimum(x+1,HEIGHT.size[0]-1)]*fx)*fy)
    return result
...
def make_stone(name,seed=713,hx=.2,hy=.15,hz=.04,res=1):
    ...
    relief=(np.minimum(scalar,.75)-.57)*.003+(broad-.5)*.0012
    side_relief=(np.minimum(scalar,.75)-.57)*.004+(noise(p,46,seed+40)-.5)*.0018
    p+=norm*(top*relief+(1-top)*side_relief)[:,None]
    edge_distance=np.minimum(hx-np.abs(raw[:,0]),hy-np.abs(raw[:,1]))
    edge_width=.003+.017*noise(p,32,seed+75)
    p[:,2]-=top*(.001+.004*noise(p,48,seed+76))*np.clip(1-edge_distance/edge_width,0,1)**.7
...
def stone_material():
    ...
    col=ramp(n,l,height_tex.outputs['Color'],'Shared sculpt field mineral interpretation',[
        (.15,(.16,.135,.095)),(.40,(.40,.32,.21)),(.53,(.44,.37,.265)),(.63,(.22,.215,.185)),(.75,(.17,.175,.152)),(.88,(.40,.33,.23))])
    ...
    detail=n.new('ShaderNodeMixRGB');detail.blend_type='MULTIPLY';detail.inputs[0].default_value=.85
    l.new(mix.outputs[0],detail.inputs[1]);l.new(image.outputs['Color'],detail.inputs[2]);l.new(detail.outputs[0],bs.inputs['Base Color'])
    ...
    bump.inputs['Distance'].default_value=.00035;bump.inputs['Strength'].default_value=.7
