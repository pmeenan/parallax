"""Native diagnostic of larger slabs using the retained three stone sources."""
...
scene=bpy.context.scene;scene.name='Large slab layout candidate2'
rng=random.Random(922813)
sources={};masters={}
for label in 'ABC':
...
# Deterministic exact-cover layout from the bounded rectangular-merge search.
N=28;STEP=1/7
layout_path=OUT.parent/'layout2.json'
layout_data=json.loads(layout_path.read_text())
assert layout_data['grid']==[N,N] and layout_data['fieldMetres']==[4,4]
tiles=[('ABC'[(i*7+item['grid'][0]*3+item['grid'][1])%3],*item['grid']) for i,item in enumerate(layout_data['tiles'])]
...
for i,(label,gx,gy,gw,gh) in enumerate(tiles):
    ...
    # This intentionally enlarges source detail for an isolated layout diagnosis.
    # It is not a production material variant or a physical grain-size proof.
...
receipt={'createdUtc':datetime.now(timezone.utc).isoformat(),'seconds':time.perf_counter()-START,'blender':bpy.app.version_string,'seed':922813,'layoutSeamScore':layout_seam_score,'layoutCandidatesScored':layout_data['candidateCount'],'layoutSource':{'path':str(layout_path),'sha256':hashlib.sha256(layout_path.read_bytes()).hexdigest()},'sources':sources,'contactRecipe':{'path':str(recipe),'sha256':hashlib.sha256(recipe.read_bytes()).hexdigest(),'material':'Compacted mixed earth contact1','nominalHeightM':.064,'localBankM':.002,'adaptation':'One welded adaptive Delaunay mesh with dense perimeter-neighborhood samples, same64mm/+2mm height formula and exact shader; conservative rectangle distance approximates actual chipped boundary. Source-only ground triangle excess restores contact continuity.'},'nominalPatchDimensionsM':[4,4],'stoneFieldDimensionsM':[4,4],'stoneCount':len(stones),'triangles':{'stones':stone_tris,'ground':ground_tris,'plants':plant_tris,'scene':sum(counts.values())},'layout':layout,'aabbOverlapPairs':overlaps,'minimumAABBClearanceM':min(clearance),'plantRoots':roots,'plantRootsInClearJoints':all(not stone_at(x,y,.0025) for x,y,z in roots),'rootDepthBelowLocalSoilM':.004,'plantVerticesConflictingWithConservativeStoneAtZBelow84mm':plant_vertex_conflicts,'actualXYScaleRange':[min(v for a in layout for v in a['scaleXYZ'][:2]),max(v for a in layout for v in a['scaleXYZ'][:2])],'aggregateCount':len(aggregate_positions),'views':views,'rightsReviewed':False,'sourceOnly':True,'runtimeQA':False,'limitations':['Only three retained mineral identities; repetition is part of this test','Provisional older plants in this layout diagnostic; approved leaf color and moss are absent','Enlarged XY scales change mineral physical grain size; this is only a slab-proportion test','Adaptive soil uses conservative rectangle proximity; original dense soil topology not copied','No compression, LOD chain, resource budget admission, shared-material compliance or engine validation'],'files':[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.iterdir()) if p.suffix in {'.png','.blend','.py'}]}
