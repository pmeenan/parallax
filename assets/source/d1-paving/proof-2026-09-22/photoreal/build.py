...
# Retain the exact builder alongside each candidate; later edits must not change its recipe.
...
# ---------------------------------------------------------------- JSON writer matching the repository's Biome format
def dump_json(obj, width=100):
    def scalar(v):
        s = json.dumps(v)
        if isinstance(v, float) and 'e' in s:  # Biome prints 1e-5, not 1e-05
            mant, exp = s.split('e')
    ...
    def fmt(v, ind, used):
        ...
        if isinstance(v, list):
            ...
            if flat and all(isinstance(x, (int, float)) and not isinstance(x, bool) for x in v):
                lines, cur = [], ''
                for p in inner:  # numeric arrays fill lines, as Biome does
...
tex.image = img_hgt
# The map contains texel-centre samples of a periodic field. Clamping samples opposite
# tile boundaries at different heights, opening cracks in the translated mesh copies.
...
write_json(os.path.join(OUT, 'layout.json'), layout)
if A.save_blend:
