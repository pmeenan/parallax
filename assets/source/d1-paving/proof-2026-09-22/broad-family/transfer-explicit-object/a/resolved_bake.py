...
CYCLE=401
OUT=Path('D:\\src\\parallax\\assets\\source\\d1-paving\\proof-2026-09-22\\broad-family\\transfer-explicit-object\\a');OUT.mkdir(exist_ok=True)
START=time.perf_counter()
source_scene=bpy.data.scenes['Broad A transfer source']
...
high=next(o for o in scene.objects if o.name.startswith('Broad source A'))
...
receipt={'createdUtc':datetime.now(timezone.utc).isoformat(),'blender':bpy.app.version_string,
 'source':'../../candidate2/source.blend','triangles':len(low.data.polygons),'vertices':len(low.data.vertices),
