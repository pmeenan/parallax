...
OUT=Path(__file__).parent/'transfer';OUT.mkdir(exist_ok=True)
BASE=OUT.parent/'candidate2/source.blend'
BASELINE=OUT.parents[2]/'surface-study-2026-09-21'
...
for index,label in enumerate('A'):
    ...
    stone=source_scene.objects['Broad source '+label]
    studio=bpy.data.scenes.new('Broad '+label+' transfer source')
    ...
    camera.location=target+Vector((1.25,-1.60,1.45));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.lens=52;camera.data.dof.use_dof=False;studio.camera=camera
    ...
    light.location=target+Vector((-3,-4,6));light.rotation_euler=(target-light.location).to_track_quat('-Z','Y').to_euler();light.data.energy=2200;light.data.size=1
    ...
    light.location=target+Vector((3,4,6));light.rotation_euler=(target-light.location).to_track_quat('-Z','Y').to_euler()
    ...
    code=code.replace("o.name.startswith('Surface study master')",f"o.name.startswith('Broad source {label}')")
    code=code.replace("'source':f'cycle{CYCLE}/source.blend'","'source':'../../candidate2/source.blend'")
    ...
    verify=verify.replace("o.name.startswith('Surface study master')",f"o.name.startswith('Broad source {label}')")
    ...
    key.location=target+Vector((3,4,6));key.rotation_euler=(target-key.location).to_track_quat('-Z','Y').to_euler()
