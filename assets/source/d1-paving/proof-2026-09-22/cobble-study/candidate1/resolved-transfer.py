"""Representative explicit moss standard GLB transfer, isolated neutral studio only."""
import bpy,json,time,hashlib,numpy as np,math
from pathlib import Path
from mathutils import Vector
OUT=folder/'transfer';OUT.mkdir(exist_ok=True);START=time.perf_counter()
bpy.ops.wm.open_mainfile(filepath=str(OUT.parent/'source.blend'));source=bpy.context.scene
targets=[source.objects[n] for n in ['Leafy moss colony 0']]
...
camera.location=center+Vector((.025,-.035,.045));camera.rotation_euler=(center-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.lens=58;camera.data.clip_start=.001
...
bpy.ops.mesh.primitive_plane_add(size=20,location=(0,0,min(v.z for v in coords)-.0002));ground=bpy.context.object;ground.name='Neutral support';mat=bpy.data.materials.new('Neutral support');mat.use_nodes=True;mat.node_tree.nodes['Principled BSDF'].inputs['Base Color'].default_value=(.07,.07,.07,1);mat.node_tree.nodes['Principled BSDF'].inputs['Roughness'].default_value=.9;ground.data.materials.append(mat)
...
export_start=time.perf_counter();bpy.ops.export_scene.gltf(filepath=str(OUT/'representative-moss.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_materials='EXPORT',export_vertex_color='NAME',export_vertex_color_name='MossColor',export_all_vertex_colors=False);export_seconds=time.perf_counter()-export_start
...
import_start=time.perf_counter();bpy.ops.import_scene.gltf(filepath=str(OUT/'representative-moss.glb'));import_seconds=time.perf_counter()-import_start
...
delta=np.abs(arrays[0]-arrays[1]);raw=(OUT/'representative-moss.glb').read_bytes();assert raw[:4]==b'glTF' and int.from_bytes(raw[8:12],'little')==len(raw)
...
r={'seconds':time.perf_counter()-START,'exportSeconds':export_seconds,'importSeconds':import_seconds,'triangles':triangles,'meshes':len(g['meshes']),'materials':len(g['materials']),'textures':len(g.get('textures',[])),'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest(),'extensions':g.get('extensionsUsed',[]),'nativeReimportPixelDelta':{'mean':float(delta.mean()),'p99':float(np.percentile(delta,99)),'max':float(delta.max())},'scope':'Representative Blender transfer only; no runtime/QA/art acceptance','notChecked':['Full glTF validator','Engine roundtrip','Compression/LOD','Production4pixel texture-class conformance','Rights clearance'],'knownDifference':'No intentional shader mismatch; explicit MossColor exported as COLOR_0. No texture maps used for moss.','files':[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.iterdir()) if p.name != 'receipt.json']}
