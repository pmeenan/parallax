...
for role,bake_type in [('color','DIFFUSE'),('normal','NORMAL')]:
    im=bpy.data.images.new('Soil bake '+role,width=4000,height=4000,alpha=True)
...
ground_arrays=[blank(4096,(1,1,1,1)),blank(4096,(.5,.5,1,1)),blank(4096,(1,.9,0,1))]
for j in range(2): ground_arrays[j][:4016,:4016]=np.pad(soil_tiles[j],((8,8),(8,8),(0,0)),mode='edge')
ground_arrays[2][:4016,:4016,1]=.94
for v in uv.data: v.uv=((v.uv.x*4000+8)/4096,(v.uv.y*4000+8)/4096)
...
for p in grit.data.polygons:
    ...
    x=4064; y=p.material_index*32; ground_arrays[0][y:y+32,x:x+32]=[*srgb(np.array(color[:3])),1]
    for li in p.loop_indices: guv.data[li].uv=((x+16)/4096,(y+16)/4096)
...
leaf_pixels=pixels(nodes['Image Texture'].image); leaf_pixels[:,:,:3]=srgb(leaf_pixels[:,:,:3])
plant_arrays[0][:528,:528]=np.pad(leaf_pixels,((8,8),(8,8),(0,0)),mode='edge')
...
plant_mat.node_tree.nodes.get('Normal Map').inputs['Strength'].default_value=.6
for ob in plants:
