...
CYCLE=303
OUT=Path('D:\\src\\parallax\\assets\\source\\d1-paving\\proof-2026-09-22\\transfer\\c');OUT.mkdir(exist_ok=True)
START=time.perf_counter()
source_scene=bpy.data.scenes['Restored C transfer source']
...
high=next(o for o in scene.objects if o.name.startswith('Family C'))
...
receipt={'createdUtc':datetime.now(timezone.utc).isoformat(),'blender':bpy.app.version_string,
 'source':'../../restored-family/source.blend','triangles':len(low.data.polygons),'vertices':len(low.data.vertices),
