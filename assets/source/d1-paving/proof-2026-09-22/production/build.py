...
assert MODE in {'candidate1','candidate2','candidate3'}
OUT=HERE/MODE
...
decimate(soil,6100 if MODE=='candidate1' else 18000)
...
chosen=sorted(np.argsort(sizes)[-100:].tolist()) if MODE=='candidate1' else list(range(len(coords)))
verts=[];faces=[];slots=[];corner_normals=[];buried_faces=0
for index in chosen:
    ...
    for p in list(d.polygons)[index*20:(index+1)*20]:
        ids=[v-index*12 for v in p.vertices]
        if MODE=='candidate3' and all(chunk[k,2]<height(*chunk[k,:2])-.0003 for k in ids):
            buried_faces+=1;continue
        faces.append(tuple(offset+k for k in ids));slots.append(p.material_index)
        corner_normals.extend([tuple(d.corner_normals[k].vector) for k in p.loop_indices])
...
if MODE=='candidate3':mesh.normals_split_custom_set(corner_normals)
grit.data=mesh
if MODE=='candidate2':decimate(grit,12000)
...
if MODE=='candidate1':
    ...
    # Preserve the source presentation outside the export without a rectangular
    # low flat ring. These faces may overlap only beneath the border stones.
    coords=[tuple(v.co) for v in soil_source.vertices]
    faces=[tuple(p.vertices) for p in soil_source.polygons
        if any(max(abs(coords[i][0]),abs(coords[i][1]))>2.0 for i in p.vertices)]
    preview.from_pydata(coords,[],faces)
preview.materials.append(soil.data.materials[0]);preview.update()
for p in preview.polygons:p.use_smooth=True
...
for ob in plants:
    if MODE=='candidate1':
        ...
        # Preserve every shoot, stem and all seven leaf silhouettes; remove only
        # their subpixel raised ridge. Isolated peripheral shoots stay rooted.
        ...
        for start in range(0,len(d.vertices),41):
            q=len(verts);verts.extend([tuple(d.vertices[start+k].co) for k in range(6)])
            tints.extend([per_vertex[start+k] for k in range(6)])
            faces.extend([(q+k,q+(k+1)%3,q+3+(k+1)%3,q+3+k) for k in range(3)])
            for leaf in range(7):
        ...
        ob.data=reduced
    else:decimate(ob,max(20,int(tris(ob)*.65)))
...
receipt={'createdUtc':__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),
    ...
    'buriedAggregateFacesRemoved':buried_faces,
