...
r={'seconds':time.perf_counter()-START,'exportSeconds':export_seconds,'importSeconds':import_seconds,'triangles':triangles,'meshes':len(g['meshes']),'materials':len(g['materials']),'textures':len(g.get('textures',[])),'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest(),'extensions':g.get('extensionsUsed',[]),'nativeReimportPixelDelta':{'mean':float(delta.mean()),'p99':float(np.percentile(delta,99)),'max':float(delta.max())},'scope':'Representative Blender transfer only; no runtime/QA/art acceptance','notChecked':['Full glTF validator','Engine roundtrip','Compression/LOD','Production4pixel texture-class conformance','Rights clearance'],'knownDifference':'No intentional shader mismatch; explicit MossColor exported as COLOR_0. No texture maps used for moss.','files':[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.iterdir())]}
