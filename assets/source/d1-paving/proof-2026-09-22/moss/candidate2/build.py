...
OUT=Path(__file__).parent; BASE=OUT.parents[1]/'leaf-color/candidate2/source.blend'; START=time.perf_counter(); rng=random.Random(922802)
...
for colony,oldcenter in enumerate(centers):
    ...
    attempts=0
    while len(roots)<260 and attempts<12000:
        ...
        fringe=max(0,1-rad/r)
        if rng.random()>(.16+.84*fringe**.45):continue
        z=soil_z(x,y);height=rng.uniform(.002,.0043)*(.48+.52*fringe**.35);lean=Vector((rng.uniform(-.00045,.00045),rng.uniform(-.00045,.00045),0));local=[];polys=[];tints=[]
        ...
        for leaf in range(7):
            ...
            length=rng.uniform(.00065,.00105)*(1-.25*t);width=length*rng.uniform(.105,.16);q=len(local)
...
camera=scene.camera;light=next(o for o in scene.objects if o.type=='LIGHT');cs=(camera.location.copy(),camera.rotation_euler.copy(),camera.data.lens);oldclip=camera.data.clip_start;camera.data.clip_start=.001;ls=(light.location.copy(),light.rotation_euler.copy(),light.data.energy,light.data.size)
controls={}
...
camera.location,camera.rotation_euler,camera.data.lens=cs;camera.data.clip_start=oldclip;light.location,light.rotation_euler,light.data.energy,light.data.size=ls
...
r={'source':str(BASE),'sourceSha256':hashlib.sha256(BASE.read_bytes()).hexdigest(),'seed':922802,'seconds':time.perf_counter()-START,'nonmossGeometryAttributesTransformsUnchanged':True,'nonmossMaterialsUnchanged':True,'nonmossHashes':before,'materialHashes':materials,'colonies':records,'sceneTriangles':total,'mossTriangles':sum(x['triangles'] for x in records),'supportHeightRangeM':[min(all_heights),max(all_heights)],'stoneVertexViolations':0,'rejectedShootCandidates':rejected,'views':views,'closeCaptureNearClipM':.001,'sourceCameraNearClipM':oldclip,'sourceOnly':True,'rightsReviewed':False,'reference':'batch-116/veg-001-courtyard-joint-plants-v1.png; moss morphology is original authored extension, not a depicted species','limits':['Plant4000tri target exceeded by explicit source microgeometry','No runtime validation, baking or LOD','Conservative stone footprint vertex tests; no full triangle collision solver'],'files':[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.iterdir()) if p.is_file()]}
