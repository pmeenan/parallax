...
OUT=Path(__file__).parent;BASE=OUT.parents[1]/'vegetation/candidate2/source.blend';START=time.perf_counter();rng=random.Random(922701)
...
factor=.83+.33*field;factor=factor*(1-main*.65)+main*.65
linear*=factor[:,:,None]
linear[:,:,0]*=1+.045*(field-.5);linear[:,:,2]*=1-.08*(field-.5)
...
for ob in scene.objects:
    ...
    for i,comp in enumerate(components):
        ...
        worn=not seedling and len(records)%7==2;blemish=not seedling and len(records)%9==5
    ...
    for loop in data.loops:
        ...
        value*=.965+.07*y
        if worn:
            edge=max(0,(abs(x-.5)*2-.63)/.37)*(.4+.6*y);value*=np.array([1+.16*edge,1+.02*edge,1-.12*edge])
        if blemish:
