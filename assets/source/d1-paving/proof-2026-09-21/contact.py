...
OUT = ROOT/'proof-2026-09-21/contact-cycle1'
OUT.mkdir(parents=True, exist_ok=True)
assert bpy.app.version[:3] == (5, 2, 1)
scene = bpy.context.scene
scene.name = 'Paving contact study 2026-09-21 cycle1'
...
def soil_height(p):
    ...
    return .064 + .003*(field(p,29,51)-.5) + .0015*(field(p,150,62)-.5) + .0008*(field(p,430,77)-.5)
...
r=n.new('ShaderNodeValToRGB'); r.color_ramp.elements[0].position=.2; r.color_ramp.elements[0].color=(.035,.019,.008,1)
r.color_ramp.elements[1].position=.8; r.color_ramp.elements[1].color=(.115,.075,.034,1)
l.new(tex.outputs['Fac'],r.inputs[0]); l.new(r.outputs[0],bs.inputs['Base Color']); bs.inputs['Roughness'].default_value=.94
bump=n.new('ShaderNodeBump'); bump.inputs['Distance'].default_value=.0008; bump.inputs['Strength'].default_value=.55
l.new(tex.outputs['Fac'],bump.inputs['Height']); l.new(bump.outputs[0],bs.inputs['Normal'])
xx,yy=np.meshgrid(np.linspace(-1.05,1.05,601),np.linspace(-1.05,1.05,601))
...
template=np.array([(1,0,0),(.38,.92,.12),(-.76,.68,-.03),(-.88,-.52,.1),(.35,-.9,-.08),(0,0,.8),(0,0,-.6)])
triangles=[(i,(i+1)%5,5) for i in range(5)]+[((i+1)%5,i,6) for i in range(5)]
for i in range(26000):
    x,y=rng.uniform(-.78,.78),rng.uniform(-.70,.70)
    if stone_at(x,y): continue
    radius=float(rng.choice([.001,.0018,.0034,.0055],p=[.40,.36,.20,.04])*rng.uniform(.65,1.3))
    z=float(soil_height(np.array([[x,y]]))[0])
    angle=rng.uniform(0,math.tau); c,s=math.cos(angle),math.sin(angle)
    rotation=np.array([[c,-s,0],[s,c,0],[0,0,1]])
    shape=template*rng.uniform(.75,1.2,(7,3)); shape[:,2]*=rng.uniform(.4,.9)
    shape=(shape@rotation.T)*radius+np.array([x,y,z-radius*.15])
...
for i,color in enumerate([(.10,.060,.026),(.15,.105,.052),(.21,.17,.11),(.082,.073,.057),(.29,.25,.18)]):
...
for poly,index in zip(grit.data.polygons,shades): poly.material_index=index
