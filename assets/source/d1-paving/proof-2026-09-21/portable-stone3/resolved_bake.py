...
CYCLE=103
OUT=Path('D:\\src\\parallax\\assets\\source\\d1-paving\\proof-2026-09-21\\portable-stone3');OUT.mkdir(exist_ok=True)
START=time.perf_counter()
source_scene=bpy.data.scenes['Paving proof stone candidate3']
...
high=next(o for o in scene.objects if o.name.startswith('Paving proof stone candidate3 master'))
...
receipt={'createdUtc':datetime.now(timezone.utc).isoformat(),'blender':bpy.app.version_string,
 'source':'../stone/candidate3/source.blend','triangles':len(low.data.polygons),'vertices':len(low.data.vertices),
