...
source=source.replace('Paving proof stone candidate3','Paving proof family candidate1')
...
def remap(src,seed):
    ...
    du=.145*np.sin(6.283*(v*1.35+.23*u)+phase[0])+.055*np.sin(6.283*(u*2.2-v*.6)+phase[1])
    dv=.135*np.sin(6.283*(u*1.17-.19*v)+phase[2])+.05*np.sin(6.283*(v*2.6+u*.7)+phase[3])
    u=(u+du+seed*.017)%1;v=(v+dv+seed*.011)%1
    x=u*(w-1);y=v*(h-1);ix=x.astype(int);iy=y.astype(int);fx=(x-ix)[:,:,None];fy=(y-iy)[:,:,None]
    return ((src[iy,ix]*(1-fx)+src[iy,np.minimum(ix+1,w-1)]*fx)*(1-fy)+(src[np.minimum(iy+1,h-1),ix]*(1-fx)+src[np.minimum(iy+1,h-1),np.minimum(ix+1,w-1)]*fx)*fy).astype(np.float32)
def packed(name,p):
...
scene.name='Paving proof family candidate1'
...
paths=[ROOT/'stone/candidate3/resolved_build.py',ROOT/'stone/candidate3/provenance.json',ROOT/'family/build_family1.py']
