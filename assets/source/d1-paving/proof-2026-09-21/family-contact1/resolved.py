...
OUT = ROOT/'proof-2026-09-21/family-contact1'
OUT.mkdir(parents=True, exist_ok=True)
assert bpy.app.version[:3] == (5, 2, 1)
scene = bpy.context.scene
scene.name = 'Paving contact study 2026-09-21 cycle3'
...
def soil_height(p):
    ...
    return .068 + .005*np.exp(-edge_distance(p)/.010)*(.55+.45*field(p,45,121)) + .005*(field(p,29,51)-.5) + .003*(field(p,150,62)-.5) + .0014*(field(p,430,77)-.5)
